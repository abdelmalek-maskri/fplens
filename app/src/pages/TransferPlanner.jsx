...
export default function TransferPlanner() {
  ...
  const outPlayer = transferOutId
    ? mockMyTeam.find((p) => p.element === transferOutId)
    : null;
    ...
    <div className="space-y-6 stagger">
      ...
      <div className="flex items-center gap-5 flex-wrap py-3 border-b border-surface-800">
        ...
        <div>
          ...
          {hitCost > 0 && (
            <span className="text-2xs text-surface-500 ml-1">after hit</span>
          )}
        </div>
      </div>
      ...
      {suggestions.length > 0 && transfers.length === 0 && (
        <div className="space-y-2">
          ...
          {suggestions.map((s) => {
            const alreadyApplied = transfers.some(
              (t) => t.out === s.out.element
            );
            return (
              <div
                key={s.out.element}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <span className="text-2xs text-danger-400 font-medium uppercase">
                      Sell
                    </span>
                ...
                {/* In */}
                <div className="flex-1 min-w-0">
                  <span className="text-2xs text-success-400 font-medium uppercase">
                    Buy
                  </span>
                  <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-4">
        ...
        {hitCost > 0 && (
          <span className="text-xs text-danger-400 ml-auto">
            Hit: -{hitCost}
          </span>
        )}
      </div>
      ...
      {!showTargets && (
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filteredTeam.map((p) => {
                const sum = p.predicted
                  .slice(0, horizon)
                  .reduce((s, v) => s + v, 0);
                const isNew = p.isTransferIn;
                const isTransferred = transfers.some(
                  (t) => t.out === p.element
                );
                return (
                  <tr
                  ...
                  >
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-2.5">
                        {isNew && (
                          <div className="w-1 h-8 rounded-full bg-success-400" />
                        )}
                        ...
                        <div>
                          ...
                          <p className="text-xs text-surface-500">
                            <span className={POSITION_COLORS[p.position]}>
                              {p.position}
                            </span>
                            {" · "}
                            {p.team}
                    ...
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-2">
                        <span className="text-surface-300 font-data tabular-nums">
                          {p.form}
                        </span>
                    ...
                    <td className="py-2.5 px-3 text-center">
                      {isNew ? (
                        <button
                          onClick={() =>
                            removeTransfer(
                              transfers.find((t) => t.in === p.element)?.out
                            )
                          }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-sm text-surface-100">
                Replace{" "}
                <span className="font-semibold">{outPlayer.web_name}</span>
              </span>
              <span className="text-xs text-surface-500">
                £
                {(Number(budget) + outPlayer.selling_price).toFixed(1)}m
                available
              </span>
            </div>
            <button
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-surface-700">
                  ...
                  {GW_LABELS.slice(0, horizon).map((gw) => (
                    <th
                      key={gw}
                      className="table-header text-center py-2.5 px-3"
                    >
                      {gw}
                    </th>
                  ))}
              ...
              <tbody>
                {availableTargets.map((t) => {
                  const outSum = outPlayer.predicted
                    .slice(0, horizon)
                    .reduce((s, v) => s + v, 0);
                  const gain = t.sum - outSum;
                  const affordable =
                    t.value <=
                    Number(budget) + outPlayer.selling_price;
                  return (
                    <tr
                      key={t.element}
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <div className="flex items-center gap-1.5">
                              ...
                              {t.price_trend === "rise" && (
                                <span className="text-2xs text-success-400">
                                  ↑
                                </span>
                              )}
                            </div>
                            <p className="text-xs text-surface-500">
                              <span className={POSITION_COLORS[t.position]}>
                                {t.position}
                              </span>
                              {" · "}
                              {t.team}
        ...
        <div className="border-t border-surface-800 pt-4">
          ...
          <div className="space-y-2 mt-3">
            {transfers.map((t) => {
              ...
              const outSum = out.predicted
                .slice(0, horizon)
                .reduce((s, v) => s + v, 0);
              const inSum = inP.predicted
                .slice(0, horizon)
                .reduce((s, v) => s + v, 0);
                ...
                (() => {
                  const gain =
                    transfers.reduce((s, t) => {
                      const out = mockMyTeam.find((p) => p.element === t.out);
                      const inP = mockTransferTargets.find(
                        (p) => p.element === t.in
                      );
                      return (
                        s +
