            ...
            return (
              <div key={m.method} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 bg-surface-800 rounded-md overflow-hidden relative">
                  <div
                    className={`h-full rounded-md ${m.best ? "bg-brand-500/60" : "bg-danger-500/40"}`}
                    style={{ width: `${Math.max(barWidth, 5)}%` }}
                  />
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-bold text-surface-100 font-data">
                    {m.mae.toFixed(4)}
                  </span>
                </div>
                {m.best && (
                  <span className="badge bg-brand-500/20 text-brand-400 shrink-0">Best</span>
                )}
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="p-3 rounded-md bg-surface-800/50 border border-surface-700">
            ...
            <p className="text-xs text-surface-400 mt-1">
              from <span className="text-surface-200 font-medium">minutes_lag1</span> alone -- last
              GW's minutes is the single strongest signal
            </p>
          </div>
          <div className="p-3 rounded-md bg-surface-800/50 border border-surface-700">
            <p className="text-lg font-bold text-warning-400">25.8%</p>
            <p className="text-xs text-surface-400 mt-1">
              from <span className="text-surface-200 font-medium">extended features</span> --
              roll10, season_avg, momentum contribute over a quarter
            </p>
          </div>
          <div className="p-3 rounded-md bg-surface-800/50 border border-surface-700">
            <p className="text-lg font-bold text-success-400">~10%</p>
            <p className="text-xs text-surface-400 mt-1">
              from <span className="text-surface-200 font-medium">static context</span> -- team,
              value, opponent, position, home/away
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {ablationConfigs.map((config) => (
          <div
            key={config.config}
            className={`p-4 border rounded-md ${config.best ? "border-info-500/30 bg-info-500/5" : config.config === "B" ? "border-brand-500/20" : "border-surface-700"}`}
          >
            <div className="flex items-center justify-between mb-3">
              <span
                className={`text-xs font-bold uppercase tracking-wider ${config.best ? "text-info-400" : config.config === "B" ? "text-brand-400" : "text-surface-400"}`}
              >
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-4">
                <div>
                  ...
                  <p className="text-sm font-semibold text-surface-300 font-data tabular-nums">
                    {config.r2.toFixed(3)}
                  </p>
                </div>
                <div>
          ...
          return (
            <div key={config.config} className="flex items-center gap-4">
              ...
              <div className="flex-1 h-8 bg-surface-800 rounded-md overflow-hidden relative">
                <div
                  className={`h-full ${config.color}/40 rounded-md`}
                  style={{ width: `${Math.max(Math.min(barWidth, 100), 5)}%` }}
                />
                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-sm font-bold text-surface-100 font-data tabular-nums">
                  {config.mae.toFixed(4)}
                </span>
              </div>
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {ablationSignificance.map((s) => (
            <div
              key={s.pair}
              className="p-3 rounded-md bg-surface-800/50 border border-surface-700"
            >
              <p className="text-sm font-medium text-surface-200">{s.pair}</p>
              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`text-sm font-bold font-data ${s.stars === "n.s." ? "text-surface-500" : "text-brand-400"}`}
                >
                  {s.stars}
                </span>
                <span className="text-xs text-surface-500">
                  p = {s.pValue < 0.001 ? "<0.001" : s.pValue.toFixed(3)}
                </span>
              </div>
        ...
        <div className="mt-3 p-3 rounded-md bg-surface-800/50 border border-surface-700">
          <p className="text-sm text-surface-300">
            <span className="font-semibold text-surface-100">
              Redundancy: {interactionEffect.redundancy.toFixed(2)}×10⁻³
            </span>{" "}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-surface-500 mt-1">
              ...
              {interactionEffect.redundancy.toFixed(2)}×10⁻³ redundancy shows Guardian sentiment
          ...
          return (
            <div key={pos.position} className="p-4 border border-surface-700 rounded-md">
              ...
              <div className="mt-3 h-2 bg-surface-800 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${pos.position === "GK" ? "bg-success-500" : pos.position === "FWD" ? "bg-danger-500" : "bg-brand-500"}`}
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          {calibrationDeciles.labels.map((label, i) => (
            <div key={label} className="flex items-center gap-3">
              <span className="text-xs text-surface-500 w-6 text-right shrink-0 font-data">
                {label}
              </span>
              <div className="flex-1 space-y-1">
                <div className="h-4 bg-surface-800 rounded overflow-hidden relative">
                  <div
                    className="h-full bg-info-500/50 rounded"
                    style={{ width: `${(calibrationDeciles.baseline[i] / maxDecile) * 100}%` }}
                  />
                  <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-2xs font-semibold text-surface-300 font-data">
                    {calibrationDeciles.baseline[i].toFixed(3)}
                  </span>
                </div>
                <div className="h-4 bg-surface-800 rounded overflow-hidden relative">
                  <div
                    className="h-full bg-brand-500/50 rounded"
                    style={{ width: `${(calibrationDeciles.stacked[i] / maxDecile) * 100}%` }}
                  />
                  <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-2xs font-semibold text-surface-300 font-data">
                    {calibrationDeciles.stacked[i].toFixed(3)}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="p-3 rounded-md bg-success-500/5 border border-success-500/20">
          ...
          <p className="text-xs text-surface-500 mt-1">
            D1-D3 MAE ranges 0.03-0.13 -- when the model is confident, it's reliable. These are
            mostly non-playing or consistently starting players.
          </p>
        </div>
        <div className="p-3 rounded-md bg-danger-500/5 border border-danger-500/20">
          <p className="text-sm font-medium text-surface-200">Edge cases remain hard</p>
          <p className="text-xs text-surface-500 mt-1">
            D10 MAE 2.6-2.7 -- high-variance players (FWDs, differentials) with unpredictable
            returns still challenge the model.
          </p>
        </div>
      </div>
    </div>
