      ...
      <div>
        ...
        <div className="space-y-2.5">
          {shapFeatures.map((f, idx) => {
            const maxPct = shapFeatures[0].importance_pct ? parseFloat(shapFeatures[0].importance_pct) : maxImportance;
                ...
                <div className="flex-1 h-7 bg-surface-800 rounded overflow-hidden relative">
                  ...
                  >
                    <span className="text-xs font-bold text-white pl-2 font-data">
                      {f.importance_pct ? `${parseFloat(f.importance_pct).toFixed(1)}%` : `${(f.importance * 100).toFixed(1)}%`}
                    </span>
                  </div>
                </div>
                <span
