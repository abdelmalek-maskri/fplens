...
def main():
    ...
    for s in seasons:
        ...
        run_module("ml.pipelines.build_fpl_fixtures_from_gws", ["--season", s])
        run_module("ml.pipelines.build_fixture_to_gw", ["--season", s])

        # 2) Fetch Understat + build required mapping tables
        run_module("ml.pipelines.fetch_understat", ["--season", s])

        # ✅ Required missing piece for understat_map_fixture:
        # writes data/processed/mappings/team_name_map_{season}.csv
        run_module("ml.pipelines.build_team_name_map", ["--season", s])

        # FPL ↔ Understat player mapping
        run_module("ml.pipelines.build_fpl_understat_mapping", ["--season", s])

        # 3) Map Understat matches to FPL fixtures, add GW, build Understat GW table
        run_module("ml.pipelines.understat_map_fixture", ["--season", s])
        run_module("ml.pipelines.understat_add_gw_by_fixture", ["--season", s])
        run_module("ml.pipelines.build_understat_gw", ["--season", s])

    # 4) Final merge across seasons (enrich FPL with Understat features)
    run_module("ml.pipelines.enrich_fpl_with_understat_all", ["--seasons"] + seasons)
